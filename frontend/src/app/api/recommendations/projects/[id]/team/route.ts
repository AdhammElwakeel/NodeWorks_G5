...
// GET /api/recommendations/projects/[id]/team
export async function GET(
  ...
  try {
    ...
    if (payload.role !== "client") {
      return NextResponse.json({ error: "Only clients can use team recommendations" }, { status: 403 });
    }
    ...
    const response = await fetch(`${AI_API_BASE_URL}/recommendations/teams`, {
      ...
      body: JSON.stringify({ projectId: id, limit, maxTeamSize, excludeUserIds }),
    });
    const result = await parseApiResponse(response);

    if (!response.ok) {
      return NextResponse.json(
        { error: result.detail || result.error || "Failed to generate team recommendations" },
