...
class KnowledgeGraphBuilder:
    ...
    def ingest_cv_data(self, cv_json):
        ...
        with self.driver.session() as session:
            ...
            """, email=cv_json['email'], name=cv_json['name'], phone=cv_json.get('phone'))
            ...
            for exp in cv_json.get('experience', []):
                ...
                """, email=cv_json['email'], company=exp['company'], role=exp['role'], years=exp.get('years'))
            ...
            for edu in cv_json.get('education', []):
                ...
                """, email=cv_json['email'], institution=edu['institution'], degree=edu['degree'])
            ...
            if 'best_role' in cv_json:
                ...
                """, email=cv_json['email'], role=cv_json['best_role'], score=cv_json.get('best_score'))

            print(f"✅ Successfully added data for {cv_json['name']} with bidirectional links.")
    ...
    if not FILE_PATH.endswith('.pdf'):
        ...
        for json_file in os.listdir('CVs Json'):
            if json_file.endswith('_result.json'):
                with open(os.path.join('CVs Json', json_file), "r", encoding="utf-8") as f:
                    data = json.load(f)
        ...
        with open("CVs Json/" + Path(FILE_PATH).stem + "_result.json", "r", encoding="utf-8") as f:
            data = json.load(f)
        kg = KnowledgeGraphBuilder()
        kg.ingest_cv_data(data)
        kg.close()
